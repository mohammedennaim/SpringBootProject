...
public class ReportServiceImpl implements ReportService {
    ...
    public OrderReportDto getOrderReport(LocalDate fromDate, LocalDate toDate) {
        ...
        try {
            ...
            report.setAverageOrderValue(
                    report.getTotalRevenue().divide(BigDecimal.valueOf(total), 2, RoundingMode.HALF_UP));
    ...
    public ShipmentReportDto getShipmentReport(LocalDate fromDate, LocalDate toDate, UUID carrierId) {
        ...
        try {
            ...
            if (total > 0) {
                report.setOnTimeDeliveryRate(
                        (report.getDeliveredShipments() - report.getDelayedShipments()) / total * 100);
            ...
            List<ShipmentReportDto.CarrierPerformance> carrierPerformances = Arrays.asList(
                    new ShipmentReportDto.CarrierPerformance("DHL001", "DHL Express", 45L, 42L, 3L, 93.3, 24.5, 85,
                            100),
                    new ShipmentReportDto.CarrierPerformance("UPS001", "UPS Standard", 35L, 30L, 4L, 85.7, 48.2, 70,
                            80),
